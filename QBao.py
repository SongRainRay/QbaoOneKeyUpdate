...
response = urllib2.urlopen(url+one_page+'1')
data = response.read().decode('utf-8')
page = int(re.findall('var totalPage = (\d*?)\;',data)[0]) #页数
#print 'total page is:',page
for i in range(1,page+1):
    response = urllib2.urlopen(url+one_page+str(i))
    data = response.read().decode('utf-8')
...
for i in range(len(suffix)):
    if suffix[i].startswith('/task'): #老版本任务
        #print suffix[i]
        response = urllib2.urlopen(url+suffix[i])
        data = response.read().decode('utf-8')
        ...
        info = {'taskId':taskId}
        sendData = urllib.urlencode(info)
        request = urllib2.Request(detail,sendData)
        ...
        if re.findall(u'任务时长：(.*?)。',data) == []:
            ...
            item.insert(7,u'视频时长：'+isVideo)
        allItems.append(item)
sortedItem = sorted(allItems, key=itemgetter(5,2),reverse=True)
header_format = '%-s,%-s,%-s,%-s,%-s,%-s,%-s,%-s\n'
header = (u'任务名'.encode('utf-8'),u'保证金/钱宝币'.encode('utf-8'),u'总收益/钱宝币'.encode('utf-8'),u'总天数'.encode('utf-8'),u'日万份收益/元'.encode('utf-8'),u'限制'.encode('utf-8'),u'备注'.encode('utf-8'),u'链接'.encode('utf-8'))
...
with open('qbao.csv', 'w') as f:
    ...
    for item in sortedItem:
        itemk = (item[1].encode('utf-8'),item[2],item[3],item[4],item[5],item[6].encode('utf-8'),item[7].encode('utf-8'),item[8].encode('utf-8'))
